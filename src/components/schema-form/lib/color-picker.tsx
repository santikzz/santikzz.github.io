// composable hsl color picker - selection area, hue/alpha sliders, eyedropper, format output
...
export const ColorPicker = ({
    ...
    const initial = Color(value ?? defaultValue);
    ...
    // only emit when the picked color actually differs from the incoming value,
    // so mounting (or StrictMode re-runs) never marks a pristine form dirty
    useEffect(() => {
        const color = Color.hsl(hue, saturation, lightness, alpha / 100);
        let current: string | null = null;
        try {
            current = value ? Color(value).hexa() : null;
        } catch {
            // unparsable incoming value - treat as no current color
        }
        if (color.hexa() === current) return;
        onChange?.(alpha < 100 ? color.hexa() : color.hex());
    }, [hue, saturation, lightness, alpha]); // eslint-disable-line react-hooks/exhaustive-deps
...
export const ColorPickerEyeDropper = ({ className, ...props }: ColorPickerEyeDropperProps) => {
    ...
    const handleEyeDropper = async () => {
        try {
            ...
            // unsupported browser or user cancelled
        }
    };
